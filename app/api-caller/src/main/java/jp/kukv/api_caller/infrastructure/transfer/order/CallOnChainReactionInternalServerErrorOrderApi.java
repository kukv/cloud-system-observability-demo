...
@Repository
class CallOnChainReactionInternalServerErrorOrderApi implements ApiCallRepository {
  ...
  public void call() {
    try {
      restClient
          ...
          .uri(
              uriBuilder ->
                  uriBuilder.path("/v1/debug/chain-reaction-internal-server-rrror").build())
          .retrieve()
          .toBodilessEntity();
    } catch (Exception _) {
      // 何もしない
  ...
  CallOnChainReactionInternalServerErrorOrderApi(
